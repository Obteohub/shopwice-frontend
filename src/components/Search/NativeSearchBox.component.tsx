...
const NativeSearchBox = () => {
  ...
  const { results, loading, error } = useSearchSuggestions(searchTerm, { perPage: 8, debounceMs: 400 });

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (wrapperRef.current && !wrapperRef.current.contains(event.target as HTMLElement)) setIsFocused(false);
    };
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (!q) return;
    setIsFocused(false);
    void router.push(`/search?q=${encodeURIComponent(q)}`);
  };
  ...
  return (
    <div ref={wrapperRef} className="w-full relative">
      <form onSubmit={handleSearchSubmit} className="relative flex w-full">
        ...
        />
        <button type="submit" title="Search" className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full bg-orange-500 p-2 text-gray-100 hover:bg-orange-600 transition-colors">
      ...
      {isFocused && searchTerm.trim().length > 2 && (
        <div className="absolute top-full left-0 mt-2 w-full rounded-xl border border-gray-100 bg-white shadow-xl z-[75] overflow-hidden">
          ...
          {!loading && !error && results.length === 0 && <div className="p-4 text-center text-gray-500 text-sm">No products found.</div>}
          {!loading && !error && results.length > 0 && (
            <ul className="divide-y divide-gray-50 max-h-[400px] overflow-y-auto">
              {results.map((item: any, index: number) => {
                ...
                const rawHref = String(source?.url || source?.href || source?.link || source?.permalink || '').trim();
                    ...
                    <Link href={href} onClick={() => setIsFocused(false)}>
                      <div className="flex items-center gap-4 p-3 hover:bg-gray-50 transition-colors">
                        <div className="h-12 w-12 flex-shrink-0 relative rounded-md overflow-hidden bg-gray-100 border border-gray-200">
                          <SuggestionImage src={imageUrl} alt={source?.title || source?.name || 'Product'} />
                        </div>
